...
export async function createBackgroundDownload({ url, filename, title, size, onProgress, onStateChange }) {
  ...
  const finishSuccess = async () => {
    ...
    if (claimed?.blob) {
      saveBlobToDevice(claimed.blob, claimed.filename || filename)
...
/**
 * Call once when a page mounts to sweep up any background downloads that finished
 * while no tab was open to receive the postMessage, and whose completion notification
 * was dismissed or never tapped. Silently saves anything it finds.
 */
export async function claimAllPendingBackgroundDownloads() {
  ...
  for (const id of Object.keys(manifest)) {
    try {
      ...
      if (result?.blob) { saveBlobToDevice(result.blob, result.filename); saved.push(result.filename) }
    } catch {}
  }
  return saved
}
