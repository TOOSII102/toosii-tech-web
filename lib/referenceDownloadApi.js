...
const ROUTES = {
  // Keep the high-bitrate /download/mp3 route out of the normal fallback chain.
  // The CDN-backed audio routes are substantially smaller for standard downloads.
  audio: ['audio', 'yta', 'ytmp3'],
  ...
  for (const route of routes) {
    try {
      ...
      return {
        ...
        quality: kind === 'audio' ? '64kbps MP3' : kind === 'spotify' ? 'MP3' : null,
        route,
      }
    } catch (error) {
