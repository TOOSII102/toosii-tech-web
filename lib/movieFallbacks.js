...
function itemsFrom(payload) {
  ...
  return [root.results, root.items, root.movies, root.shows, root.Search, root.data].find(Array.isArray) || []
}

function normalizeItem(raw, provider, forcedKind = '') {
  const value = raw?.show || raw || {}
  const sourceId = firstText(value.id, value._id, value.tmdb_id, value.imdb_id)
  ...
  const isSeries = forcedKind === 'series' || source === 'tvmaze' || String(value.type || '').toLowerCase() === 'series'
  ...
  return {
    ...
    title: String(title),
    description: stripMarkup(firstText(value.description, value.movieDescription, value.summary, value.plot, '')),
    ...
    genre,
    cover,
    countryName: firstText(value.countryName, value.country?.name, value.network?.country?.name, ''),
    imdbRatingValue: firstText(value.imdb_rating, value.imdbRating, value.rate, value.rating?.average, ''),
...
function uniqueItems(items) {
  ...
  return items.filter(item => {
    ...
    const key = `${String(item.title).toLowerCase()}|${String(item.releaseDate || '').slice(0, 4)}|${item.subjectType}`
...
async function searchFallbacks(query, type = '') {
  ...
  const loaders = series
    ? [tvmazeSearch(query), imdbSearch(query), apifySearch(query, true)]
    : [consumetSearch(query), imdbSearch(query), omdbSearch(query), apifySearch(query), allInOneMovies(), katMovies()]
...
async function catalogFallback(action) {
  ...
  const settled = await Promise.allSettled([allInOneMovies(), katMovies()])
...
export async function movieFallback({ action, q, genre = '', id, type, season, episode, title }) {
  ...
  if (['play', 'seasons', 'tv-seasons'].includes(action)) {
    ...
    return { body: brandPublicResponse({ success: true, api: 'Toosii API', provider, data: { isTV: movie.subjectType === 2, imdbId: movie.imdbId || null, tmdbId: movie.tmdbId || null, tvmazeId: movie.tvmazeId || null, seasons: details.map(item => item.number), seasonDetails: details, embedUrl: embedUrl(movie, season, episode), fallback: true } }) }
  }
