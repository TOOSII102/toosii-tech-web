...
export default function VocalRemover() {
  ...
  const requestTrack = async (track) => {
    ...
    try {
      ...
      if (mode === 'url') {
        res = await fetch(`/api/tools/vocal-remover${query}`, {
          method: 'POST',
        ...
        fd.append('file', file)
        res = await fetch(`/api/tools/vocal-remover${query}`, { method: 'POST', body: fd })
      }

      if (!res.ok) {
        const data = await res.json().catch(() => ({}))
        throw new Error(data.error || 'The track could not be generated.')
      }
      ...
      anchor.download = `${origName}_${track}.mp3`
    ...
    <Layout>
      <section className="tool-hero">
        <div className="page-wrapper">
          ...
          <h1 className="section-title">Fast <span className="gradient-text">Vocal Separation.</span> Free.</h1>
          <p className="section-sub">
            Upload any audio file or paste a direct URL — Toosii returns a clean instrumental track and isolated vocals without an account or subscription.
          </p>
        </div>
      </section>
      ...
      <section className="section" style={{ paddingTop: '1rem' }}>
        <div className="page-wrapper">
            ...
            {result && (
              <div className="tool-output" style={{ marginTop: '1.5rem' }}>
                ...
                <p style={{ color: '#94a3b8', fontSize: '0.85rem', margin: '0.5rem 0 1rem' }}>
                  Both tracks are ready — download the instrumental, the isolated vocals, or both. The files are generated by Toosii and streamed directly to your device.
                </p>
                <div style={{ display: 'flex', gap: '0.75rem', flexWrap: 'wrap' }}>
                  <button
                    type="button"
                    onClick={() => requestTrack('instrumental')}
                    disabled={Boolean(downloading)}
                    className="btn-primary"
                  >
                    {downloading === 'instrumental' ? 'Preparing instrumental…' : '🎸 Download Instrumental'}
                  </button>
                  {result.tracks?.vocal !== false && (
                    <button
                      type="button"
                      onClick={() => requestTrack('vocal')}
                      disabled={Boolean(downloading)}
                      className="btn-secondary"
                    >
                      {downloading === 'vocal' ? 'Preparing vocals…' : '🎤 Download Vocals Only'}
                    </button>
                  )}
                </div>
              </div>
          ...
          <div className="tips-grid">
            {[
              { icon: '🎙️', title: 'Vocal Separation', desc: 'A fast local-first separation pipeline keeps working even when an upstream service is unavailable.' },
